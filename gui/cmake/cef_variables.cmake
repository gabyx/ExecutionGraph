...
if(OS_LINUX)
    ...
    list(APPEND CEF_C_COMPILER_FLAGS -std=c99 # Use the C99 language standard
    )
    list(
        APPEND
    ...
    list(APPEND CEF_COMPILER_FLAGS_DEBUG -O0 # Disable optimizations
    ...
    list(
        APPEND CEF_LINKER_FLAGS_RELEASE -Wl,-O1 # Enable linker optimizations
    ...
    list(APPEND CEF_COMPILER_DEFINES_RELEASE NDEBUG # Not a debug build
    )
    ...
    if(COMPILER_SUPPORTS_NO_NARROWING)
        list(APPEND CEF_CXX_COMPILER_FLAGS -Wno-narrowing # Don't warn about type narrowing
        )
    endif()

    if(PROJECT_ARCH STREQUAL "x86_64")
        # 64-bit architecture.
        list(APPEND CEF_COMPILER_FLAGS -m64 -march=x86-64)
        list(APPEND CEF_LINKER_FLAGS -m64)
    elseif(PROJECT_ARCH STREQUAL "x86")
        # 32-bit architecture.
        list(APPEND CEF_COMPILER_FLAGS -msse2 -mfpmath=sse -mmmx -m32)
        list(APPEND CEF_LINKER_FLAGS -m32)
    endif()
    ...
    set(CEF_BINARY_FILES chrome-sandbox libcef.so natives_blob.bin snapshot_blob.bin
...
if(OS_MACOSX)
    ...
    list(APPEND CEF_COMPILER_FLAGS_RELEASE -O3 # Optimize for maximum speed plus a few extras
    )
    list(
        APPEND
        CEF_LINKER_FLAGS
    ...
    list(APPEND CEF_LINKER_FLAGS_RELEASE -Wl,-dead_strip # Strip dead code
    )
    ...
    foreach(OS_VERSION 10.13 10.12 10.11 10.10 10.9)
        set(SDK "${XCODE_PATH}/Platforms/MacOSX.platform/Developer/SDKs/MacOSX${OS_VERSION}.sdk")
        if(NOT "${CMAKE_OSX_SYSROOT}"
           AND EXISTS "${SDK}"
           AND IS_DIRECTORY "${SDK}")
    ...
    list(APPEND CEF_COMPILER_FLAGS -mmacosx-version-min=${CEF_TARGET_SDK})
...
if(OS_WINDOWS)
    ...
    if(USE_SANDBOX)
        ...
        if(${_index} EQUAL -1)
            message(
                WARNING
                    "CEF sandbox is not compatible with the current MSVC version (${MSVC_VERSION})")
            set(USE_SANDBOX OFF)
        endif()
    endif()

    # Consumers who run into LNK4099 warnings can pass /Z7 instead (see issue #385).
    set(CEF_DEBUG_INFO_FLAG
        "/Zi"
        CACHE STRING "Optional flag specifying specific /Z flag to use")
    ...
    list(
        ...
        ${CEF_DEBUG_INFO_FLAG})
    list(APPEND CEF_COMPILER_FLAGS_DEBUG /MTd # Multithreaded debug runtime
    ...
    list(APPEND CEF_LINKER_FLAGS_DEBUG /DEBUG # Generate debug information
    )
    ...
    list(
        ...
        WINVER=0x0601
        _WIN32_WINNT=0x601 # Targeting Windows 7
        NOMINMAX # Use the standard's templated min/max
        WIN32_LEAN_AND_MEAN # Exclude less common API declarations
        _HAS_EXCEPTIONS=0 # Disable exceptions
    )
    list(APPEND CEF_COMPILER_DEFINES_RELEASE NDEBUG _NDEBUG # Not a debug build
    )
    ...
    if(USE_SANDBOX)
        list(APPEND CEF_COMPILER_DEFINES PSAPI_VERSION=1 # Required by cef_sandbox.lib
             CEF_USE_SANDBOX # Used by apps to test if the sandbox is enabled
        )

        # Libraries required by cef_sandbox.lib.
        set(CEF_SANDBOX_STANDARD_LIBS dbghelp.lib psapi.lib version.lib winmm.lib)
