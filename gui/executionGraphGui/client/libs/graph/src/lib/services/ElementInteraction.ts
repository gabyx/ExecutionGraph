...
export class EventSourceGateway<TElement> implements IElementEvents<TElement> {
  ...
  public get onEnter(): Observable<ElementMouseEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onEnter))));
  }
  public get onLeave(): Observable<ElementMouseEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onLeave))));
  }
  public get onMove(): Observable<ElementMouseEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onMove))));
  }

  public get onDown(): Observable<ElementMouseButtonEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onDown))));
  }
  public get onUp(): Observable<ElementMouseButtonEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onUp))));
  }
  public get onClick(): Observable<ElementMouseButtonEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onClick))));
  }

  public get onDragStart(): Observable<ElementMouseButtonEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onDragStart))));
  }
  public get onDragContinue(): Observable<ElementMouseButtonEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onDragContinue))));
  }
  public get onDragStop(): Observable<ElementMouseButtonEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onDragStop))));
  }

  public get onScroll(): Observable<ElementMouseScrollEvent<TElement>> {
    return this.sources.pipe(switchMap(sources => merge(...sources.map(s => s.onScroll))));
  }
