...
//  template <typename C>
//  static Delegate from(C* const pObject, MemFunctionType<C> const methodFuncPtr)
//  {
//    return from<C,decltype(methodFuncPtr)>(pObject);
//  }
//
//  template <typename C>
//  static Delegate from(C const* const pObject, CMemFunctionType<C> const methodFuncPtr)
//  {
//    return from<C,decltype(methodFuncPtr)>(pObject);
//  }
//
...
//
//  template <typename C>
//  static Delegate from(C const& object, ConstMemberFuncPtr<C> const methodFuncPtr)
//  {
...
//  }
  ...
  bool operator==(Delegate const& rhs) const noexcept {return (m_pObject == rhs.m_pObject) && (m_invoker == rhs.m_invoker);}
...
private:
  ...
  DeleterFunctionPtr m_deleter; //!< Type-Erased Deleter which only calls dtor of the object inside m_functorStorage
  template <class T>
  ...
  template <typename T>
  static typename std::enable_if_t<!(is_MemberPair<T>{} || is_MemberPairConst<T>{}), R>
  functorStub(void* const pObject, Args&&... args)
  {
...
namespace std
{
  ...
  struct hash<execGraph::Delegate<R (Args...)> >
  {
    size_t operator()(execGraph::Delegate<R (Args...)> const& d) const noexcept
    {
      ...
      return hash<typename execGraph::Delegate<R (Args...)>::FunctionType>()(
        d.m_invoker) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
    }
  };
}
#endif
