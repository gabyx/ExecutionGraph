...
namespace executionGraph
{
    ...
    void FileMapper::load(const std::path& filePath)
    {
        ...
        EXECGRAPH_THROW_IF(fileDescriptor == -1, "File '{0}' could not be opened!", m_filePath);

        struct stat sb;
        EXECGRAPH_THROW_IF(::fstat(fileDescriptor, &sb) == -1,
                           "Could not read file stats '{0}",
                           m_filePath);
        EXECGRAPH_THROW_IF(!S_ISREG(sb.st_mode),
        ...
        EXECGRAPH_THROW_IF(m_mappedAddress == MAP_FAILED,
                           "File '{0}' could not be mapped in memory!",
                           m_filePath);

        EXECGRAPH_THROW_IF(::close(fileDescriptor) == -1,
